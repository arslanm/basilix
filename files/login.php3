...
if($BSX_SHOW_DOMAINS<1){
	$domain = domain2index($domain)+1;
}
$domain = (int)$domain;
...
if(!$username && !$password) {
   ...
   $err=1; 
   $domain--;
} else if((int)$domain <= 0 || (int)$domain > count($bsx_domains)) { // no valid domain selected
   $LOGIN_ERR = $lng->p(59);
...
if(!is_dir($atch_dir)) {
    $mkdcmd="/bin/mkdir -m 0700 -p $atch_dir";
    @sexec($mkdcmd);
}
